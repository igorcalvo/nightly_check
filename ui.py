...
from core import GetDescriptionText
...
def CreateCheckBoxes(descriptions: list, header: list) -> list:
    return [[(sg.Checkbox(text=PadString(' ' + item, 30),
                          ...
                          pad=((15,0),(2,2)),
                          tooltip=GetDescriptionText(descriptions, header, item)) if item != '' else sg.Text(PadString('', 37), background_color=win_bkg)) for item in splitList] for splitList in Transpose(header)]
